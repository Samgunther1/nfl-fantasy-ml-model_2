...
cfb_pred <- read_csv("data/processed/cfb_to_nfl_rb_predict_2026.csv")
nfl_pred <- read_csv("data/processed/nfl_to_nfl_rb_predict_2026.csv")
...
training_data <- read_csv("data/processed/rb_combined_training.csv")
...
draft_name_lookup <- read_csv("data/raw/cfb_draft_data.csv") %>%
  filter(!is.na(overall), position == 'Running Back') %>%
  arrange(desc(season)) %>%
...
# ── 6. Export ─────────────────────────────────────────────────────────────────
write_csv(pred_combined, "data/processed/rb_pred_combined.csv")
cat("Export complete\n")
